...
Переключается при помощи реле мгновенно, для подавление негативного эффекта, неоходимо использовать water hammer arrestor. Как и реле бывает двух видов: normally open NO (для закрытия нужно подать ток) и normally close NC (для открытия нужно подать ток).
